...
infinite:
	jal	main_state_dispatcher 	# changes the state to 32 iff state == 0 we can use this to change the state of our machine to behave differently!
					# I will show an example below, at the end of the scanner interrupt, it store 32 to the value of state
					# this next function will check if the state is 32, if it is then the machine 'knows' it is ready to move the bot!!!
...
main_state_dispatcher:
	...
	li	$t1, 32			# put 32 in $t1
	li	$t2, 0

	beq	$t0, $t1, main_move_bot_dis
	beq	$t0, $t2, state_zero
